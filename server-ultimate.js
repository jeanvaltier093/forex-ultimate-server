...
async function runScan() {
  ...
  for (const pair of PAIRS) {
    ...
    try {
      ...
      await sleep(10000); // 10s entre chaque paire — partage clé API avec serveur PROD
...
function getNextInterval() {
  ...
  if (parisHour >= 8 && parisHour < 22) return 30*60*1000; // 30 min — PROD prend les 15 min
...
async function start() {
  ...
  console.log('   ⏳ Délai de 45s avant le 1er scan (offset vs serveur PROD)');
  await loadCloud();
  app.listen(PORT, () => console.log(`🌐 Port ${PORT}`));
  // Self-ping toutes les 14 min — évite la mise en veille Render (timeout 15 min)
  setInterval(async()=>{
  ...
  await sleep(45000); // Offset de 45s pour ne pas scanner en même temps que le serveur PROD
  await runScan();
  scheduleNextScan();
}
